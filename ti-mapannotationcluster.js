...
ClusteredMapAnnotations.prototype.split = function() {
  var nextLevel = this.level + 1, subWidth = Math.round(this.bounds.width / 2), subHeight = Math
      .round(this.bounds.height / 2), latitude = Math
      .round(this.bounds.latitude), longitude = Math
      .round(this.bounds.longitude);
  this.nodes = _.map([ {
    ...
    return new ClusteredMapAnnotations(t, this.maxObjects, this.maxLevels,
        nextLevel);
  }, this);
};
...
ClusteredMapAnnotations.prototype.getIndex = function(pRect) {
  return _.min([ {
    ...
    return Math.sqrt(Math.pow(region.latitude - pRect.latitude, 2)
        + Math.pow(region.longitude - pRect.longitude, 2));
  }).index;
};

ClusteredMapAnnotations.prototype.insert = function(pRect) {
  var i = 0, index;
  var limitNear = Math.sqrt(Math.pow(this.bounds.height / 4, 2)
      + Math.pow(this.bounds.width / 4, 2));
  var tooNear = _.find(this.objects, function(objectNow) {
    var dist = Math.sqrt(Math.pow(pRect.latitude - objectNow.latitude, 2)
        + Math.pow(pRect.longitude - objectNow.longitude, 2));
    return (dist <= limitNear);
  });
  this.objects.push(pRect);
  if (((this.objects.length > this.maxObjects) || tooNear)
...
exports.fill = function(configuration) {
  ...
  var multipleAnnotationCaller = configuration.multipleAnnotationCaller
      || _.identity;
  ...
  _.each(_.filter(points, function(point) {
    return ((point.longitude) >= qy) && ((point.latitude) >= qx)
        && (point.longitude <= (qy + qh)) & (point.latitude <= (qx + qw));
  }), function(point) {
    CMA.insert(point);
  });
    ...
    if ((depth < (cma.maxLevels)) || (cma.objects.length === 1)) {
      return _.reduce(
          ...
          function(memo, location) {
            return memo.concat(map
                .createAnnotation(singleAnnotationCaller(location)));
      ...
      return [ map.createAnnotation(multipleAnnotationCaller(groupLoc,
          cma.objects)) ];
    } else {
      return [];
    }
